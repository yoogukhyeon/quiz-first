...
export default function QuizeList({ quizList, id, no, number, onClickAnswer, onClickNext, userChk, over, stepNext, score }: QuizList) {
  const router = useRouter();

  const goToResult = () => {
    router.replace(`/result?score=${score}&id=${encodeURIComponent(erypto(id))}&no=${encodeURIComponent(erypto(no))}`);
  };
...
const Label = styled.label<Correct>`
  & {
    ...
    background: ${({ correct, userClick }) =>
      correct
        ? "linear-gradient(90deg, #15f37d, #3dc47a)"
        : !correct && userClick
        ? "linear-gradient(90deg, #e93232, #b95c5c)"
        : "#ececec"};
